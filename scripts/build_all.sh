...
run_java()   { mkdir -p /tmp/java-out; while IFS= read -r -d '' f; do javac -d /tmp/java-out "$f" || { echo "FAIL $f"; exit 1; }; done < <(find "Week "* -maxdepth 2 -name "*.java" -print0); echo "Java OK"; }
